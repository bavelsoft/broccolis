...
import broccolies.annotation.FluentNestedSenders;
import static broccolies.util.AnnotationMirrorUtil.annotationMaps;
import static broccolies.util.AnnotationMirrorUtil.annotatedElements;
...
public class FluentProcessor extends AbstractProcessor {
	...
	private void generateActors(RoundEnvironment env) {
		for (Element element : env.getElementsAnnotatedWith(FluentActor.class)) {
			...
			Collection<FluentElement> enclosedElements = getEnclosedElements(element);
			try {
	...
	private Collection<FluentElement> getEnclosedElements(Element element) {
		Collection<FluentElement> enclosedElements = new ArrayList<FluentElement>();
		for (Element enclosed : element.getEnclosedElements()) {
			try {
				for (Map<String, Object> map : annotationMaps(enclosed, expecterAnnotations)) {
					enclosedElements.add(new FluentElement(
						typeElement(map.get("value")),
						false,
						typeElement(map.get("reference")),
